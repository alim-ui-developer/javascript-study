...
const getLatestNews = async () => {
  ...
  url = new URL(`https://alim-js-study.netlify.app/top-headlines?country=${COUNTRY}`);
...
const categoryFilter = async (category) => {
  ...
  url = new URL(`https://alim-js-study.netlify.app/top-headlines?country=${COUNTRY}&category=${category.toLowerCase()}`)
  const response = await fetch(url);
  const data = await response.json();
